...
Vector index on `Message.embedding` (1024-dim, cosine) for graph-RAG; retrieval fuses three legs by reciprocal rank: vector, `message\\\_text` full-text, and — when the question names a known Person/Org — that entity's own mail via `ALIAS\\\_OF`/`WORKS\\\_AT` (`graph\\\_rag.resolve\\\_anchors`). Anchored entities also get a deterministic ENTITY CARD (addresses, org, per-year counts, WITH/ABOUT split) prepended to the Ask bundle. Full-text indexes: `message\\\_text` (subject + body\\\_clean) and `person\\\_name` (Person.name), both accent-folding.

## Data layout

All runtime data lives under `data/` (gitignored). OAuth lives in `data/credentials.json` + `data/token\\\_<label>.json`. Never copy these outside `data/`. `data/claude\\\_oauth\\\_token.txt` holds the long-lived Claude Code OAuth token (`claude setup-token`, bills the Max subscription) that `_common.claude\\\_env()` injects into every `claude -p` child — headless Liam auth, managed via Settings → Claude or `POST /api/claude/token`; falls back to a shell-set `CLAUDE\\\_CODE\\\_OAUTH\\\_TOKEN`, then to the CLI's interactive login. Liam's learned state also lives here: `data/compose\\\_memory.json` + `data/ask\\\_memory.json` (long-term style/fact memory, per scope) and `data/style\\\_profiles.json` (per-recipient writing-style cards from `style\\\_profiles.py`). All are derived/rebuildable — safe to delete. `data/recipient\\\_aliases.json` is the one HAND-EDITED file here: groups of email addresses that are the same person but whose names don't auto-merge (edit it, then `POST /api/style/rebuild`).
